...
Encode::from_to($curr_changes,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
###########################

if ($curr_changes ne "+" && $curr_changes ne "-" && $curr_changes ne "" && $curr_field ne 'NOTES')
{
  my $ans="Для обозначения посещаемости необходимо использовать толлько + или -";
  ...
  #Делаем update самого посещения
