...
my $sql1="select sum(pays.SUMM_4_OPL) as plan , sum(pays.OPL_SUMM) as fact from pays, groups, branches where pays.period=? and pays.GROUP_ID=groups.ID and groups.BRANCH=branches.ID";
...
my $sql="select branches.ID as br_id, branches.BRANCH as br, sum(pays.SUMM_4_OPL) as plan, sum(pays.OPL_SUMM) as fact, pays.PERIOD as period from pays, groups, branches where pays.period=? and pays.GROUP_ID=groups.ID and groups.BRANCH=branches.ID group by branches.BRANCH";
$sth=$dbh->prepare($sql);
$sth->execute($per);
...
my $json=$op->encode({ rows=> \@result, total => $cnt, footer => [{fact=>$row[1]." руб.", plan=>$row[0]." руб.", br=>"Итого:"}]}  );
