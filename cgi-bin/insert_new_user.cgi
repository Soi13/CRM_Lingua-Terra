...
#####Создание пароля для вновь вводимого пользователя
my $RAW_PASSW=$USER_LOGIN.$year.randomPassword(3);
my $USER_PASSW=md5_hex($RAW_PASSW);

####Вычисление даты смены пароля. Это +5 дней от даты создания
my $dbh4=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
my $sth4=$dbh4->do("SET NAMES 'cp1251'");
my $sql4="select ADDDATE(NOW(),5)";
$sth4=$dbh4->prepare($sql4);
$sth4->execute();
#В массив записываем извлеченную строку
my @dt_exp_psw = $sth4->fetchrow_array;
...
if (!defined($TOP_ANKETS))
{
  ...
  my $sql1="insert into usr (LOGIN, PASSW, FULL_NAME, ADM, TOP_SEKRETARIAT, MAIN_SEKRETARIAT, GROUPS_SEKRETARIAT, STUDENTS_SEKRETARIAT, TEACHERS_SEKRETARIAT, ROOMS_SEKRETARIAT, CALENDAR_SEKRETARIAT, PERSONAL_CALENDAR_SEKRETARIAT, FIRMS_SEKRETARIAT, LINK_SEKRETARIAT, SMS_SEKRETARIAT, DOCUMENTS_SEKRETARIAT, USERS_SEKRETARIAT, ZAYAV_SEKRETARIAT, RESERV_SEKRETARIAT, OPTIONS_SEKRETARIAT, TOP_FINANCE, TRANSACTION_FINANCE, SCHET_FACTUR_FINANCE, PRIH_ORDER_FINANCE, IMPORT_FILES_BANK_FINANCE, EXPORT_FILES_BANK_FINANCE, OPTIONS_FINANCE, TOP_REGISTRATION, FORMULARS_REGISTRATION, OPTIONS_REGISTRATIONS, TOP_E_LEARNING, TESTS_E_LEARNING, LABELS_E_LEARNING, FILES_OF_BRANCH_E_LEARNING, FILES_E_LEARNING, FILES_OF_TEACHERS_E_LEARNING, TOP_MULTIMEDIATEKA, LIST_OF_POSITION_MULTIMEDIATEKA, OPTIONS_MULTIMEDIATEKA, TOP_SUPERADMIN, BRANCHES_SUPERADMIN, SUPERADMINISTRATORS_SUPERADMIN, OPTIONS_SUPERADMIN, TOP_REPORTS, REPORTS_REPORTS, DEBTS_MONTH_REPORTS, PLAN_FACT_REPORTS, PLAN_FACT_REAL_PAYS_MONTH_REPORTS, TOP_E_JOURNAL, TOP_CONTROLLER, TOP_ANKETS, ANKETS_ANKETS, LABELS_ANKETS, ABLE_DELETE_STUDENT_DOGOVOR, ABLE_DELETE_ZAYAVKI, ABLE_DELETE_GROUP, ABLE_DEL_STUDENT_FROM_GROUP, ABLE_DEL_LESSONS_FROM_GROUP, ABLE_SET_BEGIN_STUDY, ABLE_CHANGE_SUMM_4_OPL, ABLE_DEL_PERIOD_OF_PAYS, PASSW_DAYS_OF_LIVE, PASSW_DATE_EXPIRED, ACTIVE, ABLE_SEND_SMS, STUDENTS_TEACHERS_REPORTS, ABLE_ADD_PERIOD_OF_PAYS, STUDENTS_AGE_REPORTS, ABLE_ADD_EVENT, ABLE_EDIT_EVENT, ABLE_DEL_EVENT, ABLE_MOVE_STUDENTS_BETWEEN_GROUPS, STUDENTS_DOGOVORS) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1,?,?,?,?,?,?,?,?,?)";
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($USER_LOGIN, $USER_PASSW, $USER_NAME, $USER_STATUS, $TOP_SEKRETARIAT, $MAIN_SEKRETARIAT, $GROUPS_SEKRETARIAT, $STUDENTS_SEKRETARIAT, $TEACHERS_SEKRETARIAT, $ROOMS_SEKRETARIAT, $CALENDAR_SEKRETARIAT, $PERSONAL_CALENDAR_SEKRETARIAT, $FIRMS_SEKRETARIAT, $LINK_SEKRETARIAT, $SMS_SEKRETARIAT, $DOCUMENTS_SEKRETARIAT, $USERS_SEKRETARIAT, $ZAYAV_SEKRETARIAT, $RESERV_SEKRETARIAT, $OPTIONS_SEKRETARIAT, $TOP_FINANCE, $TRANSACTION_FINANCE, $SCHET_FACTUR_FINANCE, $PRIH_ORDER_FINANCE, $IMPORT_FILES_BANK_FINANCE, $EXPORT_FILES_BANK_FINANCE, $OPTIONS_FINANCE, $TOP_REGISTRATION, $FORMULARS_REGISTRATION, $OPTIONS_REGISTRATIONS, $TOP_E_LEARNING, $TESTS_E_LEARNING, $LABELS_E_LEARNING, $FILES_OF_BRANCH_E_LEARNING, $FILES_E_LEARNING, $FILES_OF_TEACHERS_E_LEARNING, $TOP_MULTIMEDIATEKA, $LIST_OF_POSITION_MULTIMEDIATEKA, $OPTIONS_MULTIMEDIATEKA, $TOP_SUPERADMIN, $BRANCHES_SUPERADMIN, $SUPERADMINISTRATORS_SUPERADMIN, $OPTIONS_SUPERADMIN, $TOP_REPORTS, $REPORTS_REPORTS, $DEBTS_MONTH_REPORTS, $PLAN_FACT_REPORTS, $PLAN_FACT_REAL_PAYS_MONTH_REPORTS, $TOP_E_JOURNAL, $TOP_CONTROLLER, $TOP_ANKETS, $ANKETS_ANKETS, $LABELS_ANKETS, $ABLE_DELETE_STUDENT_DOGOVOR, $ABLE_DELETE_ZAYAVKI, $ABLE_DELETE_GROUP, $ABLE_DEL_STUDENT_FROM_GROUP, $ABLE_DEL_LESSONS_FROM_GROUP, $ABLE_SET_BEGIN_STUDY, $ABLE_CHANGE_SUMM_4_OPL, $ABLE_DEL_PERIOD_OF_PAYS, $PASSW_DAYS_OF_LIVE, $dt_exp_psw[0], $ABLE_SEND_SMS, $STUDENTS_TEACHERS_REPORTS, $ABLE_ADD_PERIOD_OF_PAYS, $STUDENTS_AGE_REPORTS, $ABLE_ADD_EVENT, $ABLE_EDIT_EVENT, $ABLE_DEL_EVENT, $ABLE_MOVE_STUDENTS_BETWEEN_GROUPS, $STUDENTS_DOGOVORS);

  $dbh1->disconnect;

  my $ans="Пользователь добавлен успешно! <span style=\"color:red; font-weight:bold;\">Временный пароль: ".$RAW_PASSW."</span";
  #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
  my $res=$ans;
...
sub randomPassword
{
  ...
  my @chars = split(" ","Q W E R T Y U I O P L K J H G F D S A Z X C V B N M q w e r t y u i o p l k j h g f d s a z x c v b n m ! @ # $ % ^ & * _"); 
  srand;
