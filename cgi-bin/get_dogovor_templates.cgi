...
my $sql="select COUNT(*) from dogovor_templates";
$sth=$dbh->prepare($sql);
$sth->execute();
...
my $sql1="select document_templates.*, kind_doc.NAME as KND_DOC from document_templates, kind_doc where document_templates.KIND_DOCUMENT=kind_doc.ID and document_templates.TYPE_DOCUMENT=1 order by $sort $order";
