...
if ($sess->remote_addr() ne $ENV{'REMOTE_ADDR'}) {
        &f_delete_session();
        print "Location: http://linguaterra-soft.ru\n\n";
        exit();
        }
...
my $PLAN_FACT_REPORTS=$sess->param('PLAN_FACT_REPORTS');

#Смотрим определены ли логин и пароль пользователя в сесии
if (defined($formLogin) && defined($formPass)) {

    if ($PLAN_FACT_REPORTS==1) #Смотрим имеет ли доступ пользователь к отчеты план-факт по филиалам.
    {
       ...
       \$(function(){
        ...
        function make_report() {
          ...
          \$.ajax({            
                         type: 'POST',
                         traditional: true,
                         url: 'get_plan_fact_branches.cgi',
                         data: {'month':mth, 'year':yr, 'SID':'$SID'},            
        ...
        \$(function(){
           var dt= new Date();
           \$('#year').combobox('setValue', dt.getFullYear());                               
        });
        </script>
        ...
        \$(function(){
            \$('#dg').datagrid({
                ...
                onExpandRow: function(index,row){
                    ...
                    var row_br = \$('#dg').datagrid('getSelected'); 
                    var ddv = \$(this).datagrid('getRowDetail',index).find('table.ddv');                    
                    ddv.datagrid({
                        url:'get_plan_fact_branches_detail_teachers.cgi?SID=$SID&id_br='+row_br.br_id+'&per='+row_br.period,
                        method: 'get',
                        fitColumns:true,
                        singleSelect:true,
                        rownumbers:true,
                        ...
                        height:'auto',
                        columns:[[
                            {field:'teacher',title:'Преподаватель',width:90},
                            {field:'plan',title:'ПЛАН',width:100,align:'right'},
                            {field:'fact',title:'ФАКТ',width:100,align:'right'}                            
                        ]],
                        onResize:function(){
...
HTML1
        ...
        <div data-options="region:'center',title:'Рабочая область - Отчет - План-Факт по филиалам (за период). Пользователь: $formLogin'">

        <table id="dg" title="План-Факт по филиалам (за период)" class="easyui-datagrid" style="width:100%;height:95%"
        method="get"
        url=""
        toolbar="#tb"
        ...
        <thead>
        <tr>
		    <th field="br_id" width="15" hidden="true">br_id</th>
            <th field="br" width="150" sortable="true">Филиал</th>
            <th field="plan" width="167" sortable="true">ПЛАН</th>
            <th field="fact" width="167">ФАКТ</th>
            <th field="period" width="167" hidden="true">period</th>
        </tr>
        </thead>
        </table>        
        ...
        <div id="tb" style="padding:3px">
             <div style="margin-top:5px; margin-bottom:5px">
               <span>Сформировать отчет за месяц:</span>
               <select id="month" class="easyui-combobox" name="month" style="width:100px" data-options="editable:false">
               ...
               <span>    год:</span>
               <select id="year" class="easyui-combobox" name="year" style="width:100px" data-options="editable:false">
               <option>2016</option>
               <option>2017</option>
               <option>2018</option>
               <option>2019</option>
               ...
               <a id="excel" href="#" class="easyui-linkbutton" plain="true" onclick="javascript:alert('Данная функция в разработке')" style="float:right">Excel</a>
             </div>                       
        </div>
