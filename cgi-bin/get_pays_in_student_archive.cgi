...
my $sql="select COUNT(*) from pays_archive where PUPILS_NUM_DOGOVOR=?";
$sth=$dbh->prepare($sql);
$sth->execute($stud);
...
my $sql1="select ID, PERIOD, SUMM_4_OPL, OPL_SUMM, DATE_OF_PAY, STATUS from pays_archive where PUPILS_NUM_DOGOVOR=? order by (SUBSTRING_INDEX(PERIOD, '.', -1)+0), (SUBSTRING_INDEX(PERIOD, '.', 1)+0)";
