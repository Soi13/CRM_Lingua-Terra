...
my $id_topic=param('id_topic');
$id_topic=~s/\0//g; $id_topic=~s/\.\.//g; $id_topic=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
  ...
  my $sql1="delete from topic_homework where ID=?";
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($id_topic);
   
  $dbh1->disconnect;

  my $ans="Тема и ДЗ удалены успешно!";
