...
my $id_br=param('id_br');

$BRANCH=~s/\0//g; $BRANCH=~s/\.\.//g; $BRANCH=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
$id_br=~s/\0//g; $id_br=~s/\.\.//g; $id_br=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
...
if (length($BRANCH)==0)
{
  my $ans="Необходимо заполнить \"Наименование филиала\"!";
...
########Проверка длинны наименования филиала
if (length($BRANCH)>100)
{
  my $ans="Длинна наименования филиала не может быть более 100 символов!";
  #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
  my $res=$ans;
