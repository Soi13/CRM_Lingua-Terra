...
#Генерируем UUID для вставки его заявки и в расписание. По этому полю к заявке будет привязываться созданное событие в календаре
  ...
  my @row_uuid = $sth->fetchrow_array;
  $dbh->disconnect;

  if ($form_fields[4]!="0000") #Если ДР указано, то вставляем с ДР
  {
  #Вставляем саму заявку
  my $dbh1=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
  my $sth1=$dbh1->do("SET NAMES 'cp1251'");
  my $sql1="insert into zayavki (DATE_GET_ZAYAV, WHO_GET_ZAYAV, SOURCE_ZAYAV, FIO_STUDENT, FIO_PARENT, DATE_OF_BORN, LANGUAGE, LEVEL_KNOWLEDGE, BRANCH, PHONE, SOURCE_ABOUT_US, DATE_OF_TEST, TIME_OF_TEST, NOTES, USER_ID, UUID_IND, AGE_STUDENT) values (NOW(),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($form_fields[0], $form_fields[1], $form_fields[2], $form_fields[3], $form_fields[4], $form_fields[5], $form_fields[6], $form_fields[7], $form_fields[8], $form_fields[9], $form_fields[10], $form_fields[11], $form_fields[12], $us_id, $row_uuid[0], $result_age);
  }
  else #Иначе ДР не указано и вставляем без него
  {
  #Вставляем саму заявку
  my $dbh2=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
  my $sth2=$dbh2->do("SET NAMES 'cp1251'");
  my $sql2="insert into zayavki (DATE_GET_ZAYAV, WHO_GET_ZAYAV, SOURCE_ZAYAV, FIO_STUDENT, FIO_PARENT, LANGUAGE, LEVEL_KNOWLEDGE, BRANCH, PHONE, SOURCE_ABOUT_US, DATE_OF_TEST, TIME_OF_TEST, NOTES, USER_ID, UUID_IND, AGE_STUDENT) values (NOW(),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
  $sth2=$dbh2->prepare($sql2); #Для операций которые не возвращают значение
  $sth2->execute($form_fields[0], $form_fields[1], $form_fields[2], $form_fields[3], $form_fields[5], $form_fields[6], $form_fields[7], $form_fields[8], $form_fields[9], $form_fields[10], $form_fields[11], $form_fields[12], $us_id, $row_uuid[0], $result_age);
  } 
  
  #В расписание вставляем время тестирования студента указанное в заявке
  ...
  my $dbh1=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
  my $sth1=$dbh1->do("SET NAMES 'cp1251'");
  my $sql1="insert into calendar_events (START_DATE, END_DATE, TEXT_MESSAGE, DETAILS, DATETIME_CREATE, USER_ID_CREATE_EVENT, UUID_IND_ZAYAV) values (?,?,?, (select CONCAT ('Язык: ', LANGUAGE) from kind_languages where ID=?), NOW(),?,?)";
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($com_datetime, $com_datetime, $txt_message, $form_fields[5], $us_id, $row_uuid[0]);

  $dbh1->disconnect;

  my $ans="Заявка добавлена успешно!";
  #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
  my $res=$ans;
