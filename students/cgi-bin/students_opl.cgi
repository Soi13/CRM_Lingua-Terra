...
if (defined($formLogin) && defined($formPass)) {
       ...
       success: function(res) {
                                  \$('#group').text('Группа: '+res.result); //Вставляем наименование и стоимость курса                                  
                                  \$('#p').show();
                                  \$('#p').progressbar({
                                                         value: res.proc
                                                      });
...
HTML1
        ...
        <div data-options="region:'center',title:'Рабочая область - Финансы/Оплаты. Студент: $row[0]'">
        ...
        <div style="margin-top:20px; margin-left:10%;"> 		
		  <a onclick="ipayCheckout({              
              currency:'RUB',
              description: 'Лингва-Терра (Оплата за обучение)'})"
              class="easyui-linkbutton c1" style="width:200px; height:40px;">ОПЛАТИТЬ
          </a>
		</div>
