...
my $id_cr=param('id_cr');
$id_cr=~s/\0//g; $id_cr=~s/\.\.//g; $id_cr=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
...
my $sql0="select BLOCKED from kind_languages where ID=?";
$sth0=$dbh0->prepare($sql0);
$sth0->execute($id_cr);
my @state_cr = $sth0->fetchrow_array;
...
if ($state_cr[0]==1)
{
   $query="update kind_languages set BLOCKED=0 where ID=?";
   $status="активирован";
}
else
{
   $query="update kind_languages set BLOCKED=1 where ID=?";
   $status="скрыт";
}
...
my $ans="Курс/Язык ".$status." успешно!";
