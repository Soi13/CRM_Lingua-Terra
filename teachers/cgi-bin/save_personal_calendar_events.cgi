...
my @form_fields=(param('start_date'), param('end_date'), param('text'), param('details'), param('subject'), param('single_checkbox'));
...
for(my $n=0; $n<=$#form_fields; $n++ )
{
  ...
  my $sql1="insert into teachers_calendar_personal (START_DATE, END_DATE, TEXT_MESSAGE, DETAILS, USER_ID, DATETIME_CREATE, KIND_EVENT, REMINDER) values (?,?,?,?,?,NOW(),?,?)";
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($form_fields[0], $form_fields[1], $form_fields[2], $form_fields[3], $teacher_id, $form_fields[4], $form_fields[5]);
  
  $dbh1->disconnect;

  my $ans="Событие создано успешно!";
