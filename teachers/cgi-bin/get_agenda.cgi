...
my $sql="select ID, START_DATE, TEXT_MESSAGE, DETAILS, KIND_EVENT from teachers_calendar_personal where USER_ID=? and REMINDER=1 and DAY(START_DATE) like DAY(NOW()) and MONTH(START_DATE) like MONTH(NOW()) and YEAR(START_DATE) like YEAR(NOW())";
$sth=$dbh->prepare($sql);
