...
my $uuid_main2=param('uuid_main2');
$uuid_main2=~s/\0//g; $uuid_main2=~s/\.\.//g; $uuid_main2=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
...
my $sql="select COUNT(*) from past_academ_years_pays_archive where PAST_ACADEM_YEARS_UUID=?";
$sth=$dbh->prepare($sql);
$sth->execute($uuid_main2);

#В массив записываем извлеченную строку
my @row = $sth->fetchrow_array;

my $sql1="select * from past_academ_years_pays_archive where PAST_ACADEM_YEARS_UUID=?";
my $sth1=$dbh->prepare($sql1);
$sth1->execute($uuid_main2);
