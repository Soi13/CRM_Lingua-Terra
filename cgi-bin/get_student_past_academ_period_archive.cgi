...
my $number_dog=param('number_dog');
$number_dog=~s/\0//g; $number_dog=~s/\.\.//g; $number_dog=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
...
my $sql="select COUNT(*) from past_academ_years_archive where PUPILS_NUM_DOGOVOR=?";
$sth=$dbh->prepare($sql);
$sth->execute($number_dog);

#В массив записываем извлеченную строку
my @row = $sth->fetchrow_array;

my $sql1="select * from past_academ_years_archive where PUPILS_NUM_DOGOVOR=?";
my $sth1=$dbh->prepare($sql1);
$sth1->execute($number_dog);
