...
$id_tp_gr=~s/\0//g; $id_tp_gr=~s/\.\.//g; $id_tp_gr=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
...
if ($DIRECTION_STUDY eq "Лингватеррия")
{
  ...
  #Делаем update типа группы
  my $dbh1=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
  my $sth1=$dbh1->do("SET NAMES 'cp1251'");
  my $sql1="update type_groups set GROUPS=?, PRICE=?, TYPE_OPL=?, CURRENCY=?, DURATION=?, DIRECTION_STUDY=?, SHOW_COLOR=? where ID=?";
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($GROUPS, $PRICE, $TYPE_OPL, $CURRENCY, $DURATION, $DIRECTION_STUDY, $show_cl, $id_tp_gr);

  $dbh1->disconnect;

  my $ans="Тип группы отредактирован успешно!";
  #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
  my $res=$ans;
