...
##############################################################
  ...
  my $sql1="insert into rooms (ROOM_NAME, CAPACITY_ROOM, ADDRESS_ROOM, BRANCH, DESCRIPTION, LESSONS_OUT_OF_SCHOOL, USER_ID, DATETIME_CREATE) values (?,?,?,?,?,?,?, NOW())";
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($form_fields[0], $form_fields[1], $form_fields[2], $form_fields[3], $form_fields[4], $form_fields[5], $us_id);
   
  $dbh1->disconnect;

  my $ans="Кабинет добавлен успешно!";
  #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
  my $res=$ans;
