...
my $uuid_main1=param('uuid_main1');
$uuid_main1=~s/\0//g; $uuid_main1=~s/\.\.//g; $uuid_main1=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
...
my $sql="select COUNT(*) from past_academ_years_documents where PAST_ACADEM_YEARS_UUID=?";
$sth=$dbh->prepare($sql);
$sth->execute($uuid_main1);

#В массив записываем извлеченную строку
my @row = $sth->fetchrow_array;

my $sql1="select * from past_academ_years_documents where PAST_ACADEM_YEARS_UUID=?";
my $sth1=$dbh->prepare($sql1);
$sth1->execute($uuid_main1);
