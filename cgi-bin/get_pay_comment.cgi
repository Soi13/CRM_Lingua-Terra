...
#Получаем текст комментария из оплат по ID номеру
my $data;
my $dbh=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
my $sth=$dbh->do("SET NAMES 'cp1251'");
my $sql="select NOTES from pays where ID=?";
$sth=$dbh->prepare($sql);
$sth->execute($id);
my @row_notes = $sth->fetchrow_array;
#################################################

my $op=JSON->new->pretty(1);
#my $json=$op->encode(@res);
my $json=$op->encode({result => $row_notes[0]});
