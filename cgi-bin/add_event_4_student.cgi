...
my $able_add_event=$sess->param('able_add_event');

#Смотрим определены ли логин и пароль пользователя в сесии
if (defined($formLogin) && defined($formPass)) {
        
if ($able_add_event==1) #Смотрим имеет ли возможноть пользователь создавать мероприятия. Определям это по парамметру переданному из сессии.
{
my $DATE_VISIT=param('date_visit');
my $KIND_EVENT=param('kind_event');
my $FEEDBACK=param('feedback');
my $number_dog_st=param('number_dog_st');

$DATE_VISIT=~s/\0//g; $DATE_VISIT=~s/\.\.//g; $DATE_VISIT=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
$KIND_EVENT=~s/\0//g; $KIND_EVENT=~s/\.\.//g; $KIND_EVENT=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
$FEEDBACK=~s/\0//g; $FEEDBACK=~s/\.\.//g; $FEEDBACK=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
$number_dog_st=~s/\0//g; $number_dog_st=~s/\.\.//g; $number_dog_st=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
...
##############################################################
  ...
  my $sql1="insert into events_pupils (PUPILS_NUM_DOGOVOR, DATE_VISIT, KIND_EVENT, FEEDBACK, USER_ID, DATETIME_CREATE) values (?,?,?,?,?,NOW())";
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($number_dog_st, $DATE_VISIT, $KIND_EVENT, $FEEDBACK, $us_id);

  $dbh1->disconnect;

  my $ans="Мероприятие добавлено успешно!";
  #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
  my $res=$ans;
...
{
  my $ans="Вы не имеете права добавлять мероприятия!";
        #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
  my $res=$ans;
  #convert  data to JSON
  my $op = JSON -> new -> pretty(1);
