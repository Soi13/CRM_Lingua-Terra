...
my $sql="select ID, LOGIN, PASSW, PASSW_DAYS_OF_LIVE from teachers where LOGIN like ? and PASSW like ?";
$sth=$dbh->prepare($sql);
...
if ($row[1] eq $formLogin && $row[2] eq $curr_psw)
{
  ...
  my $sql2="select TEACHER_ID from teachers_old_passw where TEACHER_ID=?";
  ...
  if ($rows_count<5) 
  {
     ...
     my $dbh2=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
     my $sth2=$dbh2->do("SET NAMES 'cp1251'");
     my $sql2="select PASSW from teachers_old_passw where TEACHER_ID=? and PASSW=?";
     $sth2=$dbh2->prepare($sql2);
     ...
     {     
             ...
             #Вставляем в таблицу teachers_old_passw текущий пароль, перед заменой его на новый
             my $dbh3=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
             my $sth3=$dbh3->do("SET NAMES 'cp1251'");
             my $sql3="insert into teachers_old_passw (TEACHER_ID, PASSW, DATETIME_CREATE) values (?,?, NOW())";
             ...
             my $sql1="update teachers set PASSW=?, PASSW_DATE_EXPIRED=ADDDATE(NOW(), ?) where ID like ?";
             $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
             $sth1->execute($md5_psw, $row[3], $row[0]);
             $dbh1->disconnect;
  ...
  if ($rows_count==5)
  {
     ...
     {     
             ...
             my $sql4="select min(DATETIME_CREATE) as dt_cr from teachers_old_passw where TEACHER_ID=?";
             ...
             my $dbh3=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
             my $sth3=$dbh3->do("SET NAMES 'cp1251'");
             my $sql3="update teachers_old_passw set PASSW=?, DATETIME_CREATE=NOW() where TEACHER_ID=? and DATETIME_CREATE=?";
             $sth3=$dbh3->prepare($sql3); #Для операций которые не возвращают значение
