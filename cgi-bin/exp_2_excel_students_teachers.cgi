...
$label=~s/\0//g; $label=~s/\.\.//g; $label=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;

Encode::from_to($label,'utf8','windows-1251');
my @res_lab=split(/:/, $label);
...
$worksheet->write(1, 1, decode('windows-1251', "Студенты/Преподаватели"), $format);
$worksheet->write(3, 0, decode('windows-1251', "№Договора"), $format_tab);
$worksheet->write(3, 1, decode('windows-1251', "ФИО студента"), $format_tab);
$worksheet->write(3, 2, decode('windows-1251', "ФИО преподавателя"), $format_tab);
$worksheet->write(3, 3, decode('windows-1251', "Программа"), $format_tab);
$worksheet->write(3, 4, decode('windows-1251', "Филиал"), $format_tab);
$worksheet->write(2, 0, decode('windows-1251', "Установлен фильтр по: ".$res_lab[1]), $format_tech) if (length($label)>0);
...
for (my $h=0; $h<=$len_array-1; $h++)
{
        ...
        $worksheet->write($h+4, 2, $perl_hash->{"rows"}[$h]{"fio_teacher"}, $tabformat);
        $worksheet->write($h+4, 3, $perl_hash->{"rows"}[$h]{"prog"}, $tabformat);
        $worksheet->write($h+4, 4, $perl_hash->{"rows"}[$h]{"br"}, $tabformat);
        $cnt=$h+5;
        $count++;
}
$worksheet->write($cnt, 1, decode('windows-1251',"Кол-во:"), $tabformat);
$worksheet->write($cnt, 2, $count.decode('windows-1251', " чел."), $tabformat);
