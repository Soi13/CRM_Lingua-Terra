...
my $id_per=param('id_per');
$id_per=~s/\0//g; $id_per=~s/\.\.//g; $id_per=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;


#Определяем состояние полдзователя (заблокирован или активен). Исходя из этого будем применять нужный запрос
my $dbh0=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
my $sth0=$dbh0->do("SET NAMES 'cp1251'");
my $sql0="select FIRED from users_get_zayav where ID=?";
$sth0=$dbh0->prepare($sql0);
$sth0->execute($id_per);
my @state_per = $sth0->fetchrow_array;
...
if ($state_per[0]==1)
{
   $query="update users_get_zayav set FIRED=0 where ID=?";
   $status="активирован";
}
else
{
   $query="update users_get_zayav set FIRED=1 where ID=?";
   $status="скрыт";
}
...
my $ans="Пользователь ".$status." успешно!";
