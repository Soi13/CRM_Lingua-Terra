...
my $sql0="select BLOCKED from events where ID=?";
$sth0=$dbh0->prepare($sql0);
$sth0->execute($id_event);
my @state_event = $sth0->fetchrow_array;
...
if ($state_event[0]==1)
{
   $query="update events set BLOCKED=0 where ID=?";
   $status="активировано";
}
else
{
   $query="update events set BLOCKED=1 where ID=?";
   $status="скрыто";
}
...
$sth1->execute($id_event);

$dbh1->disconnect;

my $ans="Мероприятие ".$status." успешно!";
