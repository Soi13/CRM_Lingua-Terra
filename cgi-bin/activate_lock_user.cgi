...
my $sql0="select ACTIVE from usr where ID=?";
$sth0=$dbh0->prepare($sql0);
$sth0->execute($id_user);
my @state_user = $sth0->fetchrow_array;
...
if ($state_user[0]==1)
{
   $query="update usr set ACTIVE=0 where ID=?";
   $status="заблокирован";
}
else
{
   $query="update usr set ACTIVE=1 where ID=?";
