...
my $PERSONAL_CALENDAR_SEKRETARIAT=$sess->param('PERSONAL_CALENDAR_SEKRETARIAT');

#Смотрим определены ли логин и пароль пользователя в сесии
if (defined($formLogin) && defined($formPass)) {

    if ($PERSONAL_CALENDAR_SEKRETARIAT==1) #Смотрим имеет ли доступ пользователь к модулю "Персональный календарь".
    {
	...
	function init() {
		...
		scheduler.load("get_personal_calendar_events.cgi?SID=$SID","json");
		
		
		//Блокирование сохранения события при нажатии ENTER
		...
		scheduler.attachEvent("onEventAdded",function(id){
		    ...
		    var path="save_personal_calendar_events.cgi?SID=$SID"+"&start_date="+format(ev.start_date)+"&end_date="+format(ev.end_date)+"&text="+ev.text+"&details="+ev.details+"&subject="+ev.subject+"&single_checkbox="+ev.single_checkbox;
                    dhtmlxAjax.get(path);                   
                });
		    ...
		    var ev = scheduler.getEvent(id);
		    var format = scheduler.templates.xml_format;
		    var path="update_personal_calendar_events.cgi?SID=$SID"+"&start_date="+format(ev.start_date)+"&end_date="+format(ev.end_date)+"&text="+ev.text+"&details="+ev.details+"&id="+ev.id+"&subject="+ev.subject+"&single_checkbox="+ev.single_checkbox;
                    dhtmlxAjax.get(path);                                       
                });
		    ...
		    var ev = scheduler.getEvent(id);
		    var format = scheduler.templates.xml_format;
		    var path="delete_personal_calendar_events.cgi?SID=$SID"+"&id="+id;
                    dhtmlxAjax.get(path);                                                                                                 
                });
...
HTML1
        ...
        <div data-options="region:'center',title:'Рабочая область - Личный календарь. Пользователь: $formLogin'">
