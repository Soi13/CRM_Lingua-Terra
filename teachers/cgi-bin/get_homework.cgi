...
my $idd_gr=param('idd_gr_hw');
my $period=param('period_hw');
...
my $sql="select COUNT(*) from topic_homework where GROUP_ID=? and PERIOD=?";
$sth=$dbh->prepare($sql);
$sth->execute($idd_gr, $period);

#В массив записываем извлеченную строку
my @row = $sth->fetchrow_array;

my $sql1="select * from topic_homework where GROUP_ID=? and PERIOD=?";
my $sth1=$dbh->prepare($sql1);
$sth1->execute($idd_gr, $period);
