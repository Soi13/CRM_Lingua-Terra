...
my $sql="select COUNT(*) from past_academ_years_orders_archive where PUPILS_NUM_DOGOVOR_OR_KA=? and PAST_ACADEM_YEARS_UUID=?";
$sth=$dbh->prepare($sql);
$sth->execute($n_dog, $uuid);

#В массив записываем извлеченную строку
my @row = $sth->fetchrow_array;
my $sql1="select NUM_ORDER, OSNOVANIE, SUMM_OPL, STATUS, DATE_PAY, USER_CREATED_ORDER from past_academ_years_orders_archive where PUPILS_NUM_DOGOVOR_OR_KA=? and PAST_ACADEM_YEARS_UUID=?";
my $sth1=$dbh->prepare($sql1);
$sth1->execute($n_dog, $uuid);
