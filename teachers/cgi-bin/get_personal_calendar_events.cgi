...
my $sql="select ID as 'id', TEXT_MESSAGE as 'text', START_DATE as 'start_date', END_DATE as 'end_date', DETAILS as 'details', KIND_EVENT as 'subject', REMINDER as 'single_checkbox' from teachers_calendar_personal where START_DATE between ? and ? and END_DATE between ? and ? and USER_ID=?";
$sth=$dbh->prepare($sql);
$sth->execute($from, $to, $from, $to, $teacher_id);
#$sth->execute();
