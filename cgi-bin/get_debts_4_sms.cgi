...
my $individual_common=param('individual_common');
$fio=~s/\0//g; $fio=~s/\.\.//g; $fio=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
$n_dogovor=~s/\0//g; $n_dogovor=~s/\.\.//g; $n_dogovor=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
$teacher=~s/\0//g; $teacher=~s/\.\.//g; $teacher=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
$individual_common=~s/\0//g; $individual_common=~s/\.\.//g; $individual_common=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
...
for (my $t=0; $t<=$#row_dogovors; $t++)
{
    ...
    my $sql2="select FIO, MOBILE_PHONE from pupils where Num_dogovor=?";
    ...
    if ($common_summ>0)
    {
       ...
       push @result, {"nm_dg"=>$row_dogovors[$t],"fio"=>$fio[0], "mobile_phone"=>$fio[1], "summ"=>$common_summ};
...
my $dbh1=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
my $sth1=$dbh1->do("SET NAMES 'cp1251'");
my $sql1="select Num_dogovor from pupils where FIO=?";
$sth1=$dbh1->prepare($sql1);
$sth1->execute('%'.$fio.'%');
#В массив записываем извлеченную строку
while ($dat=$sth1->fetchrow_array) {
...
my $dbh1=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
my $sth1=$dbh1->do("SET NAMES 'cp1251'");
my $sql1="select distinct pupils_num_dogovor from pays where pupils_num_dogovor=?";
$sth1=$dbh1->prepare($sql1);
