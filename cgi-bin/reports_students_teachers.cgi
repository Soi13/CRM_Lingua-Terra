...
my $STUDENTS_TEACHERS_REPORTS=$sess->param('STUDENTS_TEACHERS_REPORTS');

#Смотрим определены ли логин и пароль пользователя в сесии
if (defined($formLogin) && defined($formPass)) {

    if ($STUDENTS_TEACHERS_REPORTS==1) #Смотрим имеет ли доступ пользователь к отчету возрасты студентов.
    {
       ...
       teacher: \$('#TEACHER_ID').combobox('getValue'),
       programm: \$('#KIND_PROGRAMM').combobox('getValue'),
       ...
       if (\$('#KIND_PROGRAMM').combobox('getValue').length>0)
       {           
           var v = \$('#KIND_PROGRAMM').combobox('getText');
           \$('#lb').text('Установлен фильтр по: '+v);
       }
       
       if (\$('#BRANCH').combobox('getValue').length>0)
       {           
           var v = \$('#BRANCH').combobox('getText');
           \$('#lb').text('Установлен фильтр по: '+v);
       }
       
       if ((\$('#BRANCH').combobox('getValue').length==0) && (\$('#KIND_PROGRAMM').combobox('getValue').length==0) && (\$('#TEACHER_ID').combobox('getValue').length==0))     
       {
           \$('#lb').text('');
       }
	...
	switch(e.keyCode){
       ...
       \$(function(){
        ...
        <script>
        function exp_2_excel_students_teachers() {
          var myData = \$('#dg').datagrid('getData'); //Получаем данные с Datagrid и передаем их в скрипт в формате JSON
          var lab=\$('#lb').text();
          \$.ajax({            
                         type: 'POST',
                         traditional: true,
                         url: 'exp_2_excel_students_teachers.cgi',
                         data: {'data':JSON.stringify(myData), 'label':lab, 'SID':'$SID'},            
	    ...
	    onSelect: function(){
	      ...
	      \$('#KIND_PROGRAMM').combobox('clear');
	    }
        });
        /////////////////////////////////////  
    
        \$('#KIND_PROGRAMM').combobox({
	    onSelect: function(){
	      \$('#BRANCH').combobox('clear');
	      \$('#TEACHER_ID').combobox('clear');
	    }
        });
        /////////////////////////////////////
...
HTML1
        ...
        <div data-options="region:'center',title:'Рабочая область - Отчет - Студенты/Преподаватели. Пользователь: $formLogin'">

        <table id="dg" title="Студенты/Преподаватели" class="easyui-datagrid" style="width:100%;height:95%"
        method="get"
        url="get_students_teachers.cgi?SID=$SID"
        toolbar="#tb"
        ...
        <div id="tb" style="padding:3px">
             ...
             <input id="TEACHER_ID" class="easyui-combobox" data-options="editable:false, selectOnNavigation:true, panelHeight:'300px', valueField: 'id', textField: 'text', url: 'get_teacher_4_lessons.cgi?SID=$SID', method: 'get', icons:[{iconCls:'icon-clear', handler: function(e){var v = \$(e.data.target).combobox('clear');} }]   ">
             <a href="#" class="easyui-linkbutton" plain="false" onclick="doSearch()" style="border-radius:5px; width:100px;">Искать</a>
             <a id="excel" href="#" class="easyui-linkbutton" plain="true" onclick="exp_2_excel_students_teachers()" style="float:right">Excel</a>
             <br />
             <span>Поиск по Программе:</span>
             <input id="KIND_PROGRAMM" class="easyui-combobox" style="width:250px;" data-options="editable:false, panelHeight:'auto', valueField: 'id', textField: 'text', url: 'get_kind_prog.cgi?SID=$SID', method: 'get', icons:[{iconCls:'icon-clear', handler: function(e){var v = \$(e.data.target).combobox('clear');} }]   ">
             <span>Поиск по Филиалу:</span>
             <input id="BRANCH" class="easyui-combobox" data-options="editable:false, panelHeight:'auto', valueField: 'id', textField: 'text', url: 'get_branches.cgi?SID=$SID', method: 'get', icons:[{iconCls:'icon-clear', handler: function(e){var v = \$(e.data.target).combobox('clear');} }]   ">
