...
my $sql="select COUNT(*) from orders where PUPILS_NUM_DOGOVOR_OR_KA=?";
$sth=$dbh->prepare($sql);
$sth->execute($n_dog);

#В массив записываем извлеченную строку
my @row = $sth->fetchrow_array;
my $sql1="select orders.ID, orders.OSNOVANIE, orders.SUMM_OPL, orders.STATUS, orders.DATE_PAY, usr.FULL_NAME as ORDER_MADE from orders, usr where orders.USER_ID=usr.ID and orders.PUPILS_NUM_DOGOVOR_OR_KA=?";
