...
my $id_src=param('id_src');
$id_src=~s/\0//g; $id_src=~s/\.\.//g; $id_src=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;


#Определяем состояние студента (заблокирован или активен). Исходя из этого будем применять нужный запрос
my $dbh0=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
my $sth0=$dbh0->do("SET NAMES 'cp1251'");
my $sql0="select BLOCKED from source_zayav where ID=?";
$sth0=$dbh0->prepare($sql0);
$sth0->execute($id_src);
my @state_src = $sth0->fetchrow_array;
...
if ($state_src[0]==1)
{
   $query="update source_zayav set BLOCKED=0 where ID=?";
   $status="активирован";
}
else
{
   $query="update source_zayav set BLOCKED=1 where ID=?";
   $status="скрыт";
}
...
my $ans="Источник заявки ".$status." успешно!";
