...
my $sql0="select BLOCKED from branches where ID=?";
$sth0=$dbh0->prepare($sql0);
$sth0->execute($id_br);
my @state_br = $sth0->fetchrow_array;
...
if ($state_br[0]==1)
{
   $query="update branches set BLOCKED=0 where ID=?";
   $status="активирован";
}
else
{
   $query="update branches set BLOCKED=1 where ID=?";
   $status="скрыт";
}
