...
my $sql="select COUNT(*) from pupils, pays where pupils.Num_dogovor=pays.PUPILS_NUM_DOGOVOR group by pupils.FIO";
$sth=$dbh->prepare($sql);
$sth->execute();

#В массив записываем извлеченную строку
my @row = $sth->fetchrow_array;
#my $sql1="select pupils.Num_dogovor, pupils.FIO, pupils.MOBILE_PHONE from pupils, pays where pupils.Num_dogovor=pays.PUPILS_NUM_DOGOVOR group by pupils.FIO order by $sort $order";
my $sql1="select pupils.Num_dogovor, pupils.FIO, pupils.MOBILE_PHONE from pupils order by $sort $order";
my $sth1=$dbh->prepare($sql1);
$sth1->execute();
