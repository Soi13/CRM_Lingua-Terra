...
my @form_fields=(param('id'), param('start_date'), param('end_date'), param('text'), param('details'), param('subject'), param('single_checkbox'));
...
for(my $n=0; $n<=$#form_fields; $n++ )
{
  ...
  my $sql1="update teachers_calendar_personal set START_DATE=?, END_DATE=?, TEXT_MESSAGE=?, DETAILS=?, KIND_EVENT=?, REMINDER=? where ID=? and USER_ID=?";
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($form_fields[1], $form_fields[2], $form_fields[3], $form_fields[4], $form_fields[5], $form_fields[6], $form_fields[0], $teacher_id);
  
  $dbh1->disconnect;

  my $ans="Событие отредактировано успешно!";
