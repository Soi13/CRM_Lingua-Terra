...
######Используется проверка данного параметра для определения производилась ли изменение на форме поля "НАименование группы", если да, то поле равно 1 и мы производим редактирование с проверкой на дублирование имени группы, если нет, то значит имя группы не менялось и оно не фигурирует в редактировании записи
if ($chd_gr_nm==1) #1, если наименование грппы редактировалось
{
...
my $sql4="select GROUP_NAME from `groups` where ID=?";
$sth4=$dbh4->prepare($sql4);
$sth4->execute($id_x);
#В массив записываем извлеченную строку
my @row_gr_name = $sth4->fetchrow_array;
...
##############################################################
  ...
  my $sql1="update groups set LANGUAGE=?, LEVEL_KNOWLEDGE=?, KIND_PROGRAMM=?, GROUP_NAME=?, GROUP_TYPE=?, TYPE_CALC_LESSON=?, COUNT_CLASSES=?, DURATION_CLASSES=?, START_DATE=?, END_DATE=Null, COURSE_PRICE=?, MIN_COUNT_STUDENTS=?, MAX_COUNT_STUDENTS=?, TYPE_PAY=?, BRANCH=? where ID=?";  
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($form_fields[0], $form_fields[1], $form_fields[2], $form_fields[3], $form_fields[4], $form_fields[5], $form_fields[6], $form_fields[7], $form_fields[8], $form_fields[9], $form_fields[10], $form_fields[11], $form_fields[12], $form_fields[13], $id_x);
        
  $dbh1->disconnect;

  my $ans="Группа отредактирована успешно!";
  #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
  my $res=$ans;
...
##############################################################
  ...
  my $dbh1=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
  my $sth1=$dbh1->do("SET NAMES 'cp1251'");
  my $sql1="update groups set LANGUAGE=?, LEVEL_KNOWLEDGE=?, KIND_PROGRAMM=?, GROUP_NAME=?, GROUP_TYPE=?, TYPE_CALC_LESSON=?, COUNT_CLASSES=?, DURATION_CLASSES=Null, START_DATE=?, COURSE_PRICE=?, MIN_COUNT_STUDENTS=?, MAX_COUNT_STUDENTS=?, TYPE_PAY=?, BRANCH=? where ID=?";  
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($form_fields[0], $form_fields[1], $form_fields[2], $form_fields[3], $form_fields[4], $form_fields[5], $form_fields[6], $form_fields[7], $form_fields[8], $form_fields[9], $form_fields[10], $form_fields[11], $form_fields[12], $id_x);
...
##############################################################
  ...
  my $dbh1=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
  my $sth1=$dbh1->do("SET NAMES 'cp1251'");
  my $sql1="update groups set LANGUAGE=?, LEVEL_KNOWLEDGE=?, KIND_PROGRAMM=?, GROUP_NAME=?, GROUP_TYPE=?, TYPE_CALC_LESSON=?, COUNT_CLASSES=Null, DURATION_CLASSES=Null, START_DATE=?, END_DATE=?, COURSE_PRICE=?, MIN_COUNT_STUDENTS=?, MAX_COUNT_STUDENTS=?, TYPE_PAY=?, BRANCH=? where ID=?";  
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($form_fields[0], $form_fields[1], $form_fields[2], $form_fields[3], $form_fields[4], $form_fields[5], $form_fields[6], $form_fields[7], $form_fields[8], $form_fields[9], $form_fields[10], $form_fields[11], $form_fields[12], $id_x);
  ...
  #Переименовываем папку на облаке
  my $client_folder = REST::Client->new();
  $client_folder->GET('http://46.180.119.231:5533/webapi/entry.cgi?api=SYNO.FileStation.Rename&version=2&method=rename&path=%5B%22%2FLinguaPlatform%2F'.$row_gr_name[0].'%22%5D&name=%5B%22'.$form_fields[3].'%22%5D&_sid='.$sid_cloud);
...
else #иначе 0, и наименование группы не редактировалось
{
########Проверка на ввод Посчитать занятия
if (length($type_calc_lesson)==0)
{
...
{
  #Создаем массив полей переданных из формы
  my @form_fields=(param('LANGUAGE'), param('LEVEL_KNOWLEDGE'), param('KIND_PROGRAMM'), param('GROUP_TYPE'), param('TYPE_CALC_LESSON'), param('COUNT_CLASSES'), param('DURATION_CLASSES'), param('START_DATE'), param('COURSE_PRICE'), param('MIN_COUNT_STUDENTS'), param('MAX_COUNT_STUDENTS'), param('TYPE_PAY'), param('BRANCH'));
...
##############################################################
  ...
  my $dbh1=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
  my $sth1=$dbh1->do("SET NAMES 'cp1251'");
  my $sql1="update groups set LANGUAGE=?, LEVEL_KNOWLEDGE=?, KIND_PROGRAMM=?, GROUP_TYPE=?, TYPE_CALC_LESSON=?, COUNT_CLASSES=?, DURATION_CLASSES=?, START_DATE=?, END_DATE=Null, COURSE_PRICE=?, MIN_COUNT_STUDENTS=?, MAX_COUNT_STUDENTS=?, TYPE_PAY=?, BRANCH=? where ID=?";  
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($form_fields[0], $form_fields[1], $form_fields[2], $form_fields[3], $form_fields[4], $form_fields[5], $form_fields[6], $form_fields[7], $form_fields[8], $form_fields[9], $form_fields[10], $form_fields[11], $form_fields[12], $id_x);
...
{
  #Создаем массив полей переданных из формы
  my @form_fields=(param('LANGUAGE'), param('LEVEL_KNOWLEDGE'), param('KIND_PROGRAMM'), param('GROUP_TYPE'), param('TYPE_CALC_LESSON'), param('COUNT_CLASSES'), param('START_DATE'), param('COURSE_PRICE'), param('MIN_COUNT_STUDENTS'), param('MAX_COUNT_STUDENTS'), param('TYPE_PAY'), param('BRANCH'));
...
########Проверка, чтобы в поле Минимальное кол-во студентов не ввели большее число чем в максимальное
if (length($form_fields[8])>0 && length($form_fields[9])>0)
{
  if ($form_fields[9]<$form_fields[8])
  {
    my $ans="В поле \"Максимальное кол-во студентов\" невозможно вводить значение меньше чем в поле \"Минимальное кол-во студентов\"!";
    #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
    my $res=$ans;
...
##############################################################
  ...
  my $dbh1=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
  my $sth1=$dbh1->do("SET NAMES 'cp1251'");
  my $sql1="update groups set LANGUAGE=?, LEVEL_KNOWLEDGE=?, KIND_PROGRAMM=?, GROUP_TYPE=?, TYPE_CALC_LESSON=?, COUNT_CLASSES=?, DURATION_CLASSES=Null, START_DATE=?, COURSE_PRICE=?, MIN_COUNT_STUDENTS=?, MAX_COUNT_STUDENTS=?, TYPE_PAY=?, BRANCH=? where ID=?";  
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($form_fields[0], $form_fields[1], $form_fields[2], $form_fields[3], $form_fields[4], $form_fields[5], $form_fields[6], $form_fields[7], $form_fields[8], $form_fields[9], $form_fields[10], $form_fields[11], $id_x);
...
{
  #Создаем массив полей переданных из формы
  my @form_fields=(param('LANGUAGE'), param('LEVEL_KNOWLEDGE'), param('KIND_PROGRAMM'), param('GROUP_TYPE'), param('TYPE_CALC_LESSON'), param('START_DATE'), param('END_DATE'), param('COURSE_PRICE'), param('MIN_COUNT_STUDENTS'), param('MAX_COUNT_STUDENTS'), param('TYPE_PAY'), param('BRANCH'));
...
########Сравнение дат, чтобы пользователь случайно не ввел дату начала меньше даты окончания
my @f = ($form_fields[5] =~ /^(\d+)\-(\d+)\-(\d+)$/);
my @e = ($form_fields[6] =~ /^(\d+)\-(\d+)\-(\d+)$/);
my $fepoch = timelocal(0,0,0,$f[2],$f[1]-1,$f[0]);
my $sepoch = timelocal(0,0,0,$e[2],$e[1]-1,$e[0]);
if ($fepoch>$sepoch)
{
...
##############################################################
  ...
  my $dbh1=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
  my $sth1=$dbh1->do("SET NAMES 'cp1251'");
  my $sql1="update groups set LANGUAGE=?, LEVEL_KNOWLEDGE=?, KIND_PROGRAMM=?, GROUP_TYPE=?, TYPE_CALC_LESSON=?, COUNT_CLASSES=Null, DURATION_CLASSES=Null, START_DATE=?, END_DATE=?, COURSE_PRICE=?, MIN_COUNT_STUDENTS=?, MAX_COUNT_STUDENTS=?, TYPE_PAY=?, BRANCH=? where ID=?";  
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($form_fields[0], $form_fields[1], $form_fields[2], $form_fields[3], $form_fields[4], $form_fields[5], $form_fields[6], $form_fields[7], $form_fields[8], $form_fields[9], $form_fields[10], $form_fields[11], $id_x);
...
######Конец проверки на редкатирвоание наименования группы на форме
