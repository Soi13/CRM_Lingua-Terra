...
my $id_ses=param('id_ses');
$id_ses=~s/\0//g; $id_ses=~s/\.\.//g; $id_ses=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;


if ($SID eq $id_ses)
{
   my $ans="Невозможно удалить собственную сессию!";
...
my $sql1="delete from sessions where id=?";
$sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
$sth1->execute($id_ses);

$dbh1->disconnect;

my $ans="Сессия удалена успешно!";
#Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
my $res=$ans;
