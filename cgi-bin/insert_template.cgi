...
##############################################################
  ...
  my $sql1="insert into document_templates (TYPE_DOCUMENT, KIND_DOCUMENT, NAIMENOVAN_DOCUMENT, DESCRIPTION_DOCUMENT, TEXT_OF_DOCUMENT, USER_ID, DATETIME_CREATE) values (?,?,?,?,?,?, NOW())";
  $sth1=$dbh1->prepare($sql1); #Для операций которые не возвращают значение
  $sth1->execute($form_fields[0], $form_fields[1], $form_fields[2], $form_fields[3], $form_fields[4], $us_id);
   
  $dbh1->disconnect;

  my $ans="Шаблон добавлен успешно!";
  #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
  my $res=$ans;
