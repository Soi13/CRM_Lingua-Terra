...
my $id_tmpl=param('id_tmpl');
$id_tmpl=~s/\0//g; $id_tmpl=~s/\.\.//g; $id_tmpl=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;

my $data;
my $dbh=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
my $sth=$dbh->do("SET NAMES 'cp1251'");

my $sql="select ID as 'id', NAIMENOVAN_DOCUMENT as 'text' from document_templates where TYPE_DOCUMENT=? order by NAIMENOVAN_DOCUMENT";
