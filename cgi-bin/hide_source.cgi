...
my $sql0="select BLOCKED from source_about_us where ID=?";
$sth0=$dbh0->prepare($sql0);
$sth0->execute($id_sr);
my @state_sr = $sth0->fetchrow_array;
...
if ($state_sr[0]==1)
{
   $query="update source_about_us set BLOCKED=0 where ID=?";
   $status="активирован";
}
else
{
   $query="update source_about_us set BLOCKED=1 where ID=?";
   $status="скрыт";
}
...
my $ans="Источник ".$status." успешно!";
