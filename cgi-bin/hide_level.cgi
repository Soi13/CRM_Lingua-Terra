...
my $id_lv=param('id_lv');
$id_lv=~s/\0//g; $id_lv=~s/\.\.//g; $id_lv=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
...
my $sql0="select BLOCKED from level_knowledge where ID=?";
$sth0=$dbh0->prepare($sql0);
$sth0->execute($id_lv);
my @state_lv = $sth0->fetchrow_array;
...
if ($state_lv[0]==1)
{
   $query="update level_knowledge set BLOCKED=0 where ID=?";
   $status="активирован";
}
else
{
   $query="update level_knowledge set BLOCKED=1 where ID=?";
   $status="скрыт";
}
