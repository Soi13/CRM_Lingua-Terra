...
{ 
   my $use_dog_fio=param('use_dog_fio');
   $use_dog_fio=~s/\0//g; $use_dog_fio=~s/\.\.//g; $use_dog_fio=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
   
    #Если режим использования ФИО и номера договора не выбран, то отправлчяем без него
   if ($use_dog_fio ne "true")
   { 
   ...
   {
      my $ans="Не введен текст СМС рассылки!";
      #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
      my $res=$ans;
   ...
   foreach my $ch (@data)
   {
        ...
        $ch=~s/\.\.//g;
        $ch=~s/<(?:[^>'"]*|(['"]).*?\1)*>//gs;
        $ch=~s/\D//gs; #Убираем из номера телефона любые символы кроме цифр.                
        
        #Проверка на длинну номера
        if (length($ch)!=11)
        {
            my$ans="Рассылка отменена! Один из номеров имеет некорректную длинну. Длинна номера должна быть 11 символов (вместе с кодом России 7). Пример: 79095556677";
            #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
            my $res=$ans;
        ...
        if (substr($ch,0,1)!=7)
        {
            my$ans="Рассылка отменена! Один из номеров имеет первую цифру отличную от 7. Все номера должны начинаться с цифры 7 (международный код России).";
            #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
            my $res=$ans;
   ...
   my @numbers;
   #Наполняем массив номерами для отправки и тегами <numbers>
   ...
   else #Иначе режим использования ФИО и №договра включен
   {
     #Получаем логин и пароль от СМС шлюза и БД
         my $dbh4=ConnectDB->connect; #Используем модуль ConnectDB для подключения к БД
         my $sth4=$dbh4->do("SET NAMES 'cp1251'");
         ...
         for (my $ch=2; $ch<=$#data; $ch=$ch+3)
         {
             $data[$ch]=~s/\D//gs; #Убираем из номера телефона любые символы кроме цифр.                
        
            #Проверка на длинну номера
            if (length($data[$ch])!=11)
            {
                my$ans="Рассылка отменена! Один или более номеров имеет некорректную длинну. Длинна номера должна быть 11 символов (вместе с кодом России 7). Пример: 79095556677 ";
                #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
                my $res=$ans;
            ...
            #Проверка на первую цифру 7 в номере телефона
            if (substr($data[$ch],0,1)!=7)
            {
                my$ans="Рассылка отменена! Один или более номеров имеет первую цифру отличную от 7. Все номера должны начинаться с цифры 7 (международный код России).";
                #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
                my $res=$ans;
         ...
         my @numbers;
         #Наполняем массив номерами для отправки и тегами <numbers>
         for (my $number=0; $number<=$#data; $number=$number+3)
         {                           
            push (@numbers, "<number variables=\"".$data[$number].";".$data[$number+1]."\">".$data[$number+2]."</number>");
         }
         #Преобразуем заполненный массив в строку для вставки его в XML структуру
         my $middle_part = join("", @numbers);
