...
if (defined($formLogin) && defined($formPass)) {
  ...
  if (@zero_opl)
  {
    my $ans="Перемещение в архив невозможно, т.к. у студента имеются не оплаченные долги! Если это не долги, например, он прекратил обучение, то лишние периоды необходимо удалить перед перемещением в архив."; 
  ...
  if (@non_opl)
  {
    my $ans="Перемещение в архив невозможно, т.к. у студента имеются не доплаченные суммы!"; 
    #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
    my $res=$ans;
  ...
  if (@homework_stud)
  {
    ...
    for (my $s=0; $s<=$#homework_stud; $s++)
    {
           ...
           $sth=$dbh->prepare($sql);
           $sth->execute($num_dog, $homework_stud[$s]->{"PERIOD"}, $homework_stud[$s]->{"DATE_LESSON"}, $homework_stud[$s]->{"TOPIC"}, $homework_stud[$s]->{"HOME_WORK"}, $us_id);
    }
    ################################################################################################
  } 
  ...
  $sth=$dbh->prepare($sql);
  $sth->execute($group_id[0], $num_dog);
  ...
  my $ans="Студент перемещен в архив удачно!";
  #Encode::from_to($ans,'utf8','windows-1251');  #Этот перекодировщик используется если данные передаются с frontend через ajax в backend и потом обратно
  my $res=$ans;
