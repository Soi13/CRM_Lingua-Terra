...
my $sql="select count(*) from groups, kind_languages, kind_programm where kind_languages.ID=groups.LANGUAGE and kind_programm.ID=groups.KIND_PROGRAMM";
$sth=$dbh->prepare($sql);
$sth->execute();

#В массив записываем извлеченную строку
my @row = $sth->fetchrow_array;
my $sql1="select groups.ID, groups.GROUP_NAME, kind_languages.LANGUAGE, kind_programm.NAME_PROGRAMM from groups, kind_languages, kind_programm where kind_languages.ID=groups.LANGUAGE and kind_programm.ID=groups.KIND_PROGRAMM order by $sort $order";
