...
my $sql="select groups.GROUP_NAME from groups, groups_students where groups.ID=groups_students.GROUP_ID and groups_students.PUPILS_NUM_DOGOVOR=?";
$sth=$dbh->prepare($sql);
$sth->execute($nm_dg);
...
if (@row_dat)
{
  ...
  my $json=$op->encode({result => $row_dat[0]});
