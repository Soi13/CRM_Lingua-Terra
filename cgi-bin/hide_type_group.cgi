...
my $sql0="select BLOCKED from type_groups where ID=?";
$sth0=$dbh0->prepare($sql0);
$sth0->execute($id_gr);
my @state_gr = $sth0->fetchrow_array;
...
if ($state_gr[0]==1)
{
   $query="update type_groups set BLOCKED=0 where ID=?";
   $status="активирован";
}
else
{
   $query="update type_groups set BLOCKED=1 where ID=?";
   $status="скрыт";
}
...
my $ans="Тип группы ".$status." успешно!";
