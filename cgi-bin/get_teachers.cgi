...
my $sql="select COUNT(*) from teachers where FIO like ? and BLOCKED<>1";
$sth=$dbh->prepare($sql);
$sth->execute('%'.$fio.'%');
...
my $sql1="select * from teachers where FIO like ? and BLOCKED<>1 order by $sort $order limit ?,?";
...
my $sql="select COUNT(*) from teachers where BLOCKED<>1";
$sth=$dbh->prepare($sql);
$sth->execute();
...
my $sql1="select * from teachers where BLOCKED<>1 order by $sort $order limit ?,?";
my $sth1=$dbh->prepare($sql1);
