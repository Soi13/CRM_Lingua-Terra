...
my $sql="select COUNT(*) from lessons where GROUP_ID=?";
$sth=$dbh->prepare($sql);
$sth->execute($gr);
...
my $sql1="select lessons.ID as ID,  teachers.FIO as FIO, rooms.ROOM_NAME as ROOM from lessons, teachers, rooms where lessons.TEACHER_ID=teachers.ID and lessons.ROOM_ID=rooms.ID and lessons.GROUP_ID=?";
