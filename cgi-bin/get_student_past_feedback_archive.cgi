...
my $uuid=param('uuid_past_feedback');
...
my $sql="select COUNT(*) from past_academ_years_feedback_lessons_archive where PAST_ACADEM_YEARS_UUID=?";
$sth=$dbh->prepare($sql);
$sth->execute($uuid);
...
my $sql1="select TOPIC, TEXT_FEEDBACK from past_academ_years_feedback_lessons_archive where PAST_ACADEM_YEARS_UUID=?";
